                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" 
                                  rows="3" 
                                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-2 md:col-span-1">
                        ...
                        @if($category->icon)
                            <p class="mt-1 text-xs text-gray-500">
                                Current icon preview: <i class="{{ $category->icon }} text-indigo-600 ml-1"></i>
                            </p>
                        @endif
                        @error('icon')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="col-span-2 md:col-span-1">
                        ...
                        @if($category->image)
                            <div class="mb-3 p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-xs text-gray-500">{{ basename($category->image) }}</p>
                                        <p class="text-xs text-gray-400 mt-1">Upload new image to replace</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <div id="image-preview" class="mt-3 hidden">
                            ...
                            <img src="" alt="Preview" class="h-20 w-20 object-cover rounded-lg border border-gray-200">
                        </div>
                    ...
                    <div class="col-span-2">
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {{-- Featured Order --}}
                                <div class="col-span-2 md:col-span-1 featured-order-field" style="{{ old('is_featured', $category->is_featured) ? '' : 'display: none;' }}">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">
                                        Featured Order
                                    </label>
                                    <input type="number" 
                                           ...
                                           value="{{ old('featured_order', $category->featured_order ?? 0) }}" 
                                           min="0">
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="meta_description" 
                                  rows="2" 
                                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('meta_description') border-red-500 @enderror">{{ old('meta_description', $category->meta_description) }}</textarea>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                if (!validTypes.includes(file.type)) {
                    alert('Invalid file type. Please upload JPEG, PNG, JPG, GIF, or WebP image.');
                    this.value = '';
                    previewContainer.classList.add('hidden');
                    previewImage.src = '';
                    return;
    ...
    // Toggle featured order field based on checkbox
    ...
    const featuredOrderInput = document.querySelector('input[name="featured_order"]');

    if (featuredCheckbox && featuredOrderField && featuredOrderInput) {
        featuredCheckbox.addEventListener('change', function() {
            if (this.checked) {
                featuredOrderField.style.display = 'block';
            } else {
                featuredOrderField.style.display = 'none';
                featuredOrderInput.value = 0;
            }
        });
    }
    ...
    if (metaDescription) {
        ...
        const updateDescCounter = () => {
            ...
            descCounter.textContent = `${length} characters (recommended: 150-160)`;
            if (length > 160) {
                descCounter.classList.add('text-yellow-600');
            } else {
                descCounter.classList.remove('text-yellow-600');
            }
        };
        
        metaDescription.addEventListener('input', updateDescCounter);
        updateDescCounter(); // Initial call
    }
    ...
    // Also mark as dirty on checkbox changes
...
/* Drag and drop area hover effect */
.border-dashed:hover {
    border-color: #6366f1;
...
/* Smooth transition for featured order field */
